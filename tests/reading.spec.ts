...
test('a litany pairs every invocation with its exact response in compact columns', async ({
	...
	// A narrower phone and enlarged type stack the same complete pairs before
	// an indivisible Latin-and-gloss token can overlap or scroll sideways.
	const compactShape = () =>
		page
			.locator('.litany-pair')
			.first()
	...
	await page.evaluate(() => localStorage.setItem('scrutabor-reading', 'larger'));
	await page.setViewportSize({ width: 390, height: 760 });
	await page.goto('/app/pl/litaniae/lauretanae');
	await expect.poll(compactShape).toEqual({ columnCount: 1, pageOverflow: 0 });
});
