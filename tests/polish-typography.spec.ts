...
// every one of them for hydration and for the webfont bought nothing. The
// pages this DOES operate wait for themselves, below. The network guard is
// the reason it still goes through a fixture at all.
import { bare as test, expect, settled } from './fixtures';

// One context that walks the entire sitemap slowly accumulates enough page
// state to make a late hydration depend on the runner's memory pressure.
// Playwright gives every test a fresh context, so fixed interleaved batches
// keep the coverage exhaustive while bounding the state any one context can
// retain. Interleaving also spreads the large reading routes across batches.
const SWEEP_BATCHES = 32;
...
for (let batch = 0; batch < SWEEP_BATCHES; batch += 1) {
	test(`no Polish surface leaves a one-letter word before a breakable space ${batch + 1}/${SWEEP_BATCHES} @online @sweep`, async ({
		page,
		request
	}) => {
		...
		allPaths.push('/pl/404');
		expect(allPaths.length).toBeGreaterThan(150);
		const paths = allPaths.filter((_, index) => index % SWEEP_BATCHES === batch);
		...
		for (const path of paths) {
			...
			await test.step(path, async () => {
				...
				// Only a page about to be OPERATED has to be alive. A fresh batch
				// normally hydrates in milliseconds; thirty seconds still tolerates
				// a loaded runner without turning one stuck document into minutes of
				// ambiguous silence.
				if (operable) await settled(page, 30_000);
