...
const AHEAD = dayToday(new Date(2027, 4, 6)); // Ascension, not written
const STRANDED = dayToday(new Date(2027, 4, 11)); // a feria after an unwritten Sunday
...
describe('the hint under the day picker', () => {
	it('reads the three shapes of today the calendar can return', () => {
		...
		expect(AHEAD.on?.formulary, 'which the calendar can still name').toBe('ascensio-domini');
