...
const AHEAD = dayToday(new Date(2027, 3, 4)); // Low Sunday, not written
const STRANDED = dayToday(new Date(2027, 3, 6)); // a feria in an unwritten Easter week
...
describe('the hint under the day picker', () => {
	it('reads the three shapes of today the calendar can return', () => {
		...
		expect(AHEAD.on?.formulary, 'which the calendar can still name').toBe('dominica-in-albis');
